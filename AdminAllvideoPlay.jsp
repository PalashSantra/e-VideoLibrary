...
Email_Id:<%=session.getAttribute("Email") %>
    ...
    <table>
		<tr>
			...
			<tr>
				<td><a href='<c:url value="servPlayVideo"><c:param name="n2" value="${row.sFileName}"/></c:url>'>${row.sFileName}</a></td>
				<td>${VF.ProperQuantity(row.lSizeByte)}</td>
				<td><a href='<c:url value="servPlayVideo"><c:param name="n2" value="${row.sFileName}"/></c:url>'>Play</a></td>
